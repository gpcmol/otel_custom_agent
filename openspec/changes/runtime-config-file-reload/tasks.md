...
- [x] 1.2 Add file-based configuration loading with the documented precedence over `OTEL_CUSTOM_AGENT_CONFIG` while preserving no-configuration startup behavior.
...
- [x] 2.4 Ensure unchanged content is not reparsed or republished and that webserver updates continue to use the existing runtime path.

## 3. Verification

- [x] 3.1 Add tests for source precedence, environment fallback, absent sources, default interval, and custom interval validation.
- [x] 3.2 Add tests for valid reloads, unchanged files, invalid or temporarily unreadable files, and shutdown cleanup.
- [x] 3.3 Add or update documentation and deployment examples for both environment variables, including Kubernetes ConfigMap usage and precedence.
